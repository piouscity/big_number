...
string DecToBin(const QInt &other)
{
	string result = "";

	for (int i = 0; i < qSize; i++)
	{
		result = result + char(other.GetBit(i) + '0');
	}
...
QInt BinToDec(const string &bit)
{
	...
	{
		result.SetBit(length - i - 1, bool(bit[i] - '0'));
	}
...
string BinToHex(const string &bit)
{
	...
	for (int i = L_bit - 1, j = 0; i >= 0; i--, j++)
	{
		...
		if (j == 3 || i == 0)
		{
			if (num < 10)
			{
				result = result + char(num + '0');
			}
			else
			{
				result = result + char(num - 10 + 'A');
			}
...
/// </summary>
/// <returns> So thap luc phan sau khi chuyen doi </returns>
string DecToHex(const QInt &other)
{
	string bit = DecToBin(other);
...
{
	int L_hexa = hexa.size();
	string bit = "";

	for (int i = L_hexa - 1; i >= 0; i--)
	{
		int num;
